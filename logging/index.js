    ...
    static startup() {
        ...
        server.on("message", (gelf) => {
            ...
            if (logMatch.test(gelf.short_message)) {
                ...
                appInsights.defaultClient.trackRequest({tagOverrides: {"ai.location.ip": ipaddress}, name: `${method} ${url}`, url: `${serverport === "443" ? "https" : "http"}://${servername}${url}`, duration: +requesttime * 1000, resultCode: statuscode, success: successMatch.test(statuscode), time: new Date(date), properties: {application: "nnn", container: gelf._container_name || "nnn-logging", ipaddress, remoteuser, serverport: +serverport, request, http, bytessent: +bytessent, referrer, useragent}});
            } else {
                appInsights.defaultClient.trackTrace({message: gelf.short_message, properties: {application: "nnn", container: gelf._container_name || "nnn-logging"}});
            }
        });
